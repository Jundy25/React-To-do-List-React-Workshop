...
function App() {
  ...
  const renderTask = (task) => (
    <div key={task.id} className="flex flex-col border rounded-lg py-3 px-3 gap-4 hover:bg-gray-100"
      onClick={() => openModal("view", task)}>
      <div className="relative flex flex-row items-start">
        {task.completed ? (
          // GREEN CHECK
          <svg
            className="w-6 h-6 min-w-6 mr-1 cursor-pointer hover:fill-green-900"
            onClick={(e) => 
              {
                e.stopPropagation(); 
          ...
          // EMPTY CHECK
            <svg
              ...
              strokeWidth="1.5"
              stroke="currentColor"
              className="w-6 h-6 min-w-6 mr-1 cursor-pointer hover:fill-gray-400"
              onClick={(e) => 
                {
                  e.stopPropagation(); 
                  toggleComplete(task.id)}}
        ...
        <p
          className={`text-xs py-1 px-3 rounded-full cursor-default ${
            task.priority === 'low'
        ...
        <div className="w-full ml-1">
          <p className={`text-wrap font-medium flex-grow break-words cursor-default${task.completed ? "line-through" : ""}`}>{task.title}</p>
        </div>
        <div className="flex ml-2">
          <button onClick={(e) => {e.stopPropagation(); openModal("edit", task)}} className="mr-1 hover:cursor-pointer transition duration-300 ease-in-out hover:-translate-y-1 hover:scale-110">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
            </svg>
          </button>
          
          <button onClick={(e) => {e.stopPropagation(); handleDeleteTask(task.id)}} className="hover:cursor-pointer transition duration-300 ease-in-out hover:-translate-y-1 hover:scale-110">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
      ...
      <p className="text-xs break-words cursor-default">{task.description}</p>
    </div>
  );

  // Function to render a time-of-day section
  const renderTimeSection = (title, tasks, icon) => (
    <div className="flex flex-col justify-start bg-white w-full h-fit border rounded-lg py-6 px-4 gap-4 mb-6">
